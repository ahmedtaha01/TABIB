<?php
 ...
 if(isset($_GET['num'])){
    $_SESSION['page_number_admin_reviews'] = $_GET['num'];
 }

  if(!isset($_SESSION['page_number_admin_reviews'])){      //check for the first entry there is no session
   $_SESSION['page_number_admin_reviews'] = 1;             // so error will be established
  }                                                            //of course first entry means page 1
 if(isset($_GET['state']) && $_GET['state'] == 'previous'){
   if($_SESSION['page_number_admin_reviews'] != 1){              

      $_SESSION['page_number_admin_reviews'] =  $_SESSION['page_number_admin_reviews'] -1;                                     
 ...
 $offset = $items_per_page * ($_SESSION['page_number_admin_reviews']-1);              //start
